...
class Rl2::Thread::Downloader
  ...
  def dat_download_first_time http, dat_path, path
    ...
    response = http.get( dat_path, { 'Accept-Encoding' => 'gzip' } )
    throw 'DAT can not download.' if response.code.to_i >= 300
    
    modified_time = Time.httpdate(response['Last-Modified'])
    ...
    # File.utime( modified_time, modified_time, path )
  end
  ...
  def dat_download_second_or_later http, dat_path, path
    ...
    response = http.get( dat_path, {
      #'If-Modified-Since' => modified_time.httpdate,
